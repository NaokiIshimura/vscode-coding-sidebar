...
| Claude Code自動検知 | ターミナル出力からClaude Codeの起動・終了を自動検知してショートカットボタンを切り替え |
...
2. コマンドラインからインストール:
   ```bash
   code --install-extension ai-coding-sidebar-0.8.22.vsix
   ```
3. VS Codeを再起動
...
code --install-extension releases/ai-coding-sidebar-0.8.22.vsix
```
