...
| アーカイブ | タスクディレクトリをアーカイブしてワークスペースを整理。ディレクトリ行のアーカイブアイコン（インラインボタン）をクリックするか、右クリックして「Archive」を選択すると、`archived`フォルダに移動されます。同名のディレクトリが既に存在する場合は、競合を避けるために自動的にタイムスタンプが追加されます。 |
...
2. コマンドラインからインストール:
   ```bash
   code --install-extension ai-coding-sidebar-0.7.31.vsix
   ```
3. VS Codeを再起動

#### ローカルビルド版を使用する場合:
```bash
# releasesディレクトリから直接インストール（バージョン0.7.31）
code --install-extension releases/ai-coding-sidebar-0.7.31.vsix
```
