...
| **Directory List** | ディレクトリリストを表示<br>AIコーディングでマークダウンを配置するディレクトリ配下を表示できる<br>settingsでデフォルトのパスを設定可能 |
| **Markdown List** | マークダウンリストを表示<br>AIコーディングで利用するmarkdownを作成できる |
| **File Changes** | 変更されたファイルを一覧表示し、差分を確認 |
| **Settings** | 設定 |
